...
게임 플레이 화면입니다.
좌 우 방향키를 눌러 플레이어(파란색 상자)를 움직입니다.
Esc 키를 눌러 게임 도중에 종료할 수 있습니다.
![23b28e50ebd8bfd5](https://github.com/user-attachments/assets/161296bd-f110-4305-87c6-6a588c371857)


점수는 생존시간 n초마다 1씩 증가합니다(현재 점수 증가 조건을 5초로 지정하였고, 수정 가능)
남은 생명은 게임 플레이시 3부터 시작합니다. 장애물(빨간색 상자)에 닿을 때마다 1씩 감소하고 0보다 낮을 시 게임 오버 화면이 출력됩니다.
생존 시간은 초단위로 출력됩니다.
스테이지는 일정 점수마다 1씩 증가합니다.(현재 스테이지 증가 조건을 점수가 5점일 때로 지정하였고, 수정 가능)
스테이지가 증가 할수록 장애물(빨간색 상자)의 크기와 낙하 속도, 생성 빈도가 조정 됩니다.
생명 아이템(초록색 상자)에 닿으면 남은 생명 수가 1 증가 합니다.(생명은 최대 5까지 증가 시킬 수 있음, 수정 가능) 
![3a9bf9d9f6c2bb36](https://github.com/user-attachments/assets/83a0d97e-5f46-4191-babf-510ba2c6702e)


게임오버 화면입니다.
총 점수와 총 생존시간이 함께 출력됩니다.
시작 화면과 같이 Space, Esc 기능을 사용할 수 있습니다.
