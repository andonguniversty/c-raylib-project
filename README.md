...
| 일차 | 날짜 | 주요 작업 | 참여 |
|------|------|----------|------|
| 1일차 | 05.14 | 프로젝트 설정, Raylib 기본 창 구성, 게임 루프 구현, GameManager 클래스 설계 | 전재민, 김무진, 김준용 |
| 2일차 | 05.15 | Player 클래스 구현 (이동, 그리기, 경계), 장애물/아이템 기본 클래스 설계 | 전재민, 김무진 |
| 3일차 | 05.16 | 장애물/아이템 생성 및 이동 로직 구현, 화면 밖 객체 제거 | 김무진, 김준용 |
| 4일차 | 05.17 | 충돌 감지 로직 구현 (플레이어-장애물/아이템), 생명 시스템 구현, 게임 오버 상태 전환 | 전재민, 김무진 |
| 5일차 | 05.18 | 점수/시간/스테이지 시스템 구현, 난이도 조절 로직 적용, UI 구현 (점수, 생명, 시간, 스테이지, 게임 오버 화면), 재시작/종료 기능 구현, 최종 테스트 및 디버깅 | 전재민, 김준용, 김무진 |
...
| 게임 플레이 화면 예시                             | 게임 오버 화면 예시                              |
| :------------------------------------------------ | :----------------------------------------------- |
|  |  |

*(참고: 위의 `https://www.google.com/search?q=URL_OF_YOUR_GAME_PLAY_IMAGE` 및 `https://www.google.com/search?q=URL_OF_YOUR_GAME_OVER_IMAGE` 부분을 실제 이미지 파일의 웹 주소로 변경해야 합니다. 일반적으로 GitHub 저장소의 `image` 폴더 등에 이미지를 넣고 해당 파일의 Raw 또는 Permalinks 주소를 사용합니다.)*
